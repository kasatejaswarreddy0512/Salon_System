...
import {
  ...
  FETCH_SERVICE_BY_SALON_FAILURE,
  FETCH_SERVICE_BY_SALON_REQUEST,
  FETCH_SERVICE_BY_SALON_SUCCESS,
...
export const createService =
  ...
  async (dispatch) => {
    ...
    try {
      const { data } = await api.post(`${BASE_URL}/salon-owner`, service, {
  ...
  async (dispatch) => {
    ...
    try {
      const { data } = await api.put(
        `${BASE_URL}/salon-owner/${serviceId}`,
        service,
      );
